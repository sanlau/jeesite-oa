...
//		return ProcessDiagramGenerator.generateDiagram(bpmnModel, "png", activeActivityIds);
	...
	public Map<String, Object> getProcessVariables(String taskId) {
		...
		{
			Task t=taskService.createTaskQuery().taskId(taskId).includeProcessVariables().singleResult();
			if(t==null)
			{
				return historyService.createHistoricTaskInstanceQuery().taskId(taskId).includeProcessVariables().singleResult().getProcessVariables();
			}else
			{
				return t.getProcessVariables();
			}
		}
	}
	...
	//获取已办任务
	public List<Act> propose(Act act) {
		// TODO Auto-generated method stub
		String userId = UserUtils.getUser().getLoginName();//ObjectUtils.toString(UserUtils.getUser().getId());
		List<Act> result=new ArrayList<Act>();
		...
		// 查询列表
				List<HistoricProcessInstance> histList = histTaskQuery.list();
		// 设置查询条件
		if (StringUtils.isNotBlank(act.getProcDefKey())){
			histTaskQuery.processDefinitionKey(act.getProcDefKey());
		}
		...
		for (HistoricProcessInstance historicProcessInstance : histList) {
			Act e = new Act();
			e.setHistoticProcessInstance(historicProcessInstance);
			e.setVars(historicProcessInstance.getProcessVariables());
			e.setProcDef(ProcessDefCache.get(historicProcessInstance.getProcessDefinitionId()));
			result.add(e);
		}
